...
#pragma omp parallel shared(dna_sequence, motif, chunk_size, found) default(none)
  {
#pragma omp single
    {
      for (size_t i = 0; i < dna_sequence.size() && not found; i += chunk_size) {
#pragma omp task shared(dna_sequence, motif, chunk_size, found) firstprivate(i) default(none)
        {
          if (dna_sequence.substr(i == 0 ? 0 : i - motif.length(), i + chunk_size)
